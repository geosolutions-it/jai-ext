...
/**
 * A class representing a request for non-prefetch background computation of tiles. The object stores the image, the indices of all tiles being
...
class Request implements TileRequest {
     ...
     * 
     * @exception IllegalArgumentException if <code>scheduler</code>, <code>image</code>, or <code>tileIndices</code> is <code>null</code> or if
     *            <code>tileIndices</code> is zero-length.
     */
    Request(TileScheduler scheduler, PlanarImage image, Point[] tileIndices,
...
interface Job {
    ...
    /**
     * Returns <code>true</code> if and only if the job should block the thread which processes it. In this case the scheduler and the processing
     * thread must communicate using <code>wait()</code> and <code>notify()</code>.
     */
    boolean isBlocking();
...
/**
 * A <code>Job</code> which computes a single tile at a time for a non-prefetch background job queued by the version of scheduleTiles() which returns
 * a <code>TileRequest</code>. This <code>Job</code> notifies all <code>TileComputationListener</code>s of all <code>TileRequest</code>s with which
 * this tile is associated of whether the tile was computed or the computation failed.
 */
final class RequestJob implements Job {

    final JAIExtTileScheduler scheduler; // the TileScheduler
    ...
    /** Constructor. */
    RequestJob(JAIExtTileScheduler scheduler, PlanarImage owner, int tileX, int tileY,
    ...
    public void compute() {
        ...
        synchronized (scheduler.tileRequests) {
            ...
            Object tileID = JAIExtTileScheduler.tileKey(owner, tileX, tileY);
        ...
        if (reqList != null && !reqList.isEmpty()) {
            ...
            try {
                ...
                Set listeners = JAIExtTileScheduler.getListeners(reqList);
                ...
                if (listeners != null && !listeners.isEmpty()) {
                    ...
                    tileStatus = new Integer(exception == null ? TileRequest.TILE_STATUS_COMPUTED
                    ...
                    if (exception == null) {
                        ...
                        while (iter.hasNext()) {
                            TileComputationListener listener = (TileComputationListener) iter
                                    .next();
                            listener.tileComputed(scheduler, requests, owner, tileX, tileY,
                                    tiles[offset]);
                        }
                    } else {
                        ...
                        while (iter.hasNext()) {
                            TileComputationListener listener = (TileComputationListener) iter
                                    .next();
                            listener.tileComputationFailure(scheduler, requests, owner, tileX,
                                    tileY, exception);
                        }
                    }
                }
    ...
    /**
     * Returns <code>true</code> if the job is not done; that is, the tile is not computed and no exceptions have occurred.
     */
    ...
    public String toString() {
        ...
        return getClass().getName() + "@" + Integer.toHexString(hashCode()) + ": owner = "
                + owner.toString() + " tileX = " + Integer.toString(tileX) + " tileY = "
                + Integer.toString(tileY) + " tile = " + tString;
    }
}
...
final class TileJob implements Job {
    ...
    /** Constructor. */
    TileJob(JAIExtTileScheduler scheduler, boolean isBlocking, PlanarImage owner,
            Point[] tileIndices, Raster[] tiles, int offset, int numTiles) {
        this.scheduler = scheduler;
...
class WorkerThread extends Thread {
    ...
    JAIExtTileScheduler scheduler;

    /** Whether this is a prefetch thread. */
    boolean isPrefetch;

    /** Constructor. */
    public WorkerThread(ThreadGroup group, JAIExtTileScheduler scheduler, boolean isPrefetch) {
    ...
    public void run() {
        ...
        while (true) {
            ...
            if (dequeuedObject == TERMINATE || getThreadGroup() == null
...
/**
 * This is another implementation of the <code>javax.media.jai.TileScheduler</code> interface. It provides a mechanism for scheduling tile
 * calculation. Multi-threading is used whenever possible.
 * 
...
public class JAIExtTileScheduler implements TileScheduler {
    ...
    /**
     * The tile scheduler name. It is used to compose the name of the ThreadGroup.
     */
    private static String name = JaiI18N.getString("JaiExtTileScheduler_Name");

    /**
     * The root ThreadGroup, which holds two sub-groups: the ThreadGroup for the standard jobs, and the ThreadGroup for the prefetch jobs.
     */
    ...
    /**
     * A <code>Vector</code> of <code>WorkerThread</code>s that persist to do the actual tile computation for normal processing. This variable should
     * never be set to <code>null</code>.
     */
    private Vector workers = new Vector();

    /**
     * A <code>Vector</code> of <code>WorkerThread</code>s that persist to do the actual tile computation for prefetch processing. This variable
     * should never be set to <code>null</code>.
     */
    private Vector prefetchWorkers = new Vector();

    /**
     * The effective number of worker threads; may differ from <code>workers.size()</code> due to latency. This value should equal the size of
     * <code>workers</code> less the number of <code>WorkerThread.TERMINATE</code>s in <code>queue</code>.
     */
    private int numWorkerThreads = 0;

    /**
     * The effective number of prefetch worker threads; may differ from <code>prefetchWorkers.size()</code> due to latency. This value should equal
     * the size of <code>prefetchWorkers</code> less the number of <code>WorkerThread.TERMINATE</code>s in <code>prefetchQueue</code>.
     */
    private int numPrefetchThreads = 0;

    /**
     * <code>Map</code> of tiles currently being computed. The key is created from the image and tile indices by the <code>tileKey()</code> method.
     * Each key is mapped to an <code>Object[1]</code> which may contain <code>null</code>, a <code>Raster</code>, or an indefinite
     * <code>Object</code> which represent, respectively, that the tile is being computed, the tile itself, and that the tile computation failed.
     */
    private Map tilesInProgress = new HashMap();

    /**
     * <code>Map</code> of tiles to <code>Request</code>s. The key is created from the image and tile indices by the <code>tileKey()</code> method.
     * Each key is mapped to a <code>List</code> of <code>Request</code> for the tile. If there is no mapping for the tile, then there are no current
     * requests. If a mapping exists, it should always be non-null and the <code>List</code> value should have size of at least unity.
     */
    Map tileRequests = new HashMap();

    /**
     * <code>Map</code> of tiles to <code>Job</code>s.The key is created from the image and tile indices by the <code>tileKey()</code> method. Each
     * key is mapped to a <code>Job</code> for the tile. If there is no mapping for the tile, then there is no enqueued <code>RequestJob</code>.
     */
    ...
    /**
     * Returns the hash table "key" as a <code>Object</code> for this tile. For <code>PlanarImage</code> and <code>SerializableRenderedImage</code>,
     * the key is generated by the method <code>ImageUtilgenerateID(Object) </code>. For the other cases, a <code>Long</code> object is returned. The
     * upper 32 bits for this <code>Long</code> is the tile owner's hash code, and the lower 32 bits is the tile's index.
     */
     ...
     * Constructor.
     * 
     * @param parallelism The number of worker threads to do tile computation. If this number is less than 1, no multi-threading is used.
     * @param priority The priority of worker threads.
     * @param prefetchParallelism The number of threads to do prefetching. If this number is less than 1, no multi-threading is used.
     * @param prefetchPriority The priority of prefetch threads.
     */
    public JAIExtTileScheduler(int parallelism, int priority, int prefetchParallelism,
     ...
     * Tile computation. Does the actual calls to getTile().
     */
    Exception compute(PlanarImage owner, Point[] tileIndices, Raster[] tiles, int offset,
        ...
        if (request == null || request.listeners == null) {
            ...
            for (int i = 0; i < numTiles; i++, j++) {
                ...
                /*
                 * XXX try { List reqList; synchronized(tileRequests) { Long tileID = tileKey(owner, p.x, p.y); reqList =
                 * (List)tileRequests.remove(tileID); tileJobs.remove(tileID); } if(reqList != null) { tiles[j] = owner.getTile(p.x, p.y);
                 * TileRequest[] reqs = (TileRequest[])reqList.toArray(new TileRequest[0]); Set listeners = getListeners(reqList); if(listeners !=
                 * null) { Iterator iter = listeners.iterator(); while(iter.hasNext()) { TileComputationListener listener =
                 * (TileComputationListener)iter.next(); listener.tileComputed(this, reqs, owner, p.x, p.y, tiles[j]); } } } } catch (Exception e) {
                 * exception = e;
                 * 
                 * // Abort the remaining tiles in the job. break; }
                 */
            }
        }
        ...
        /*
         * XXX if(exception != null) { int numFailed = numTiles - (j - offset); for(int i = 0; i < numFailed; i++) { Point p = tileIndices[j++]; Long
         * tileID = tileKey(owner, p.x, p.y); List reqList = (List)tileRequests.remove(tileID); tileJobs.remove(tileID); if(reqList != null) {
         * TileRequest[] reqs = (TileRequest[])reqList.toArray(new TileRequest[0]); Set listeners = getListeners(reqList); if(listeners != null) {
         * Iterator iter = listeners.iterator(); while(iter.hasNext()) { TileComputationListener listener = (TileComputationListener)iter.next();
         * listener.tileComputationFailure(this, reqs, owner, p.x, p.y, exception); } } } } }
         */
        ...
        if (computeTile) {
            try {
                ...
                if (e instanceof Error) {
                    ...
                    sendExceptionToListener(JaiI18N.getString("JaiExtTileScheduler_5"), e);
                    // throw (RuntimeException)e;
                } else {
                    String message = JaiI18N.getString("JaiExtTileScheduler_5");
            ...
            synchronized (cache) {
                ...
                if (cache[0] instanceof Raster) {
                    ...
                    throw new RuntimeException(JaiI18N.getString("JaiExtTileScheduler_4"));
                }
            }
        }
    ...
    /**
     * General purpose method for job creation and queueing. Note that the returned value should be ignored if the <code>listener</code> parameter is
     * non-<code>null</code>.
     * 
    ...
    private Object scheduleJob(PlanarImage owner, Point[] tileIndices, boolean isBlocking,
            boolean isPrefetch, TileComputationListener[] listeners) {
        if (owner == null || tileIndices == null) {
            // null parameters
            throw new IllegalArgumentException(JaiI18N.getString("JaiExtTileScheduler_0")); // coding error - no message
            ...
            if (numThreads > 0) { // worker threads exist
                if (numTiles <= numThreads || // no more tiles than threads
                    ...
                    if (!isBlocking && !isPrefetch) {
                        ...
                        while (numJobs < numTiles) {
                            ...
                            synchronized (tileRequests) {
                                ...
                                if (tileRequests.containsKey(tileID)) {
                                    ...
                                    jobs[numJobs] = new RequestJob(this, owner, p.x, p.y, tiles,
                                            numJobs);
                        ...
                        while (numJobs < numTiles) {
                            jobs[numJobs] = new TileJob(this, isBlocking, owner, tileIndices,
                                    tiles, numJobs, 1);
                    ...
                    // number of tiles.
                    int minTilesPerThread = numThreads == 1 ? numTiles : Math.min(
                            Math.max(1, (int) (frac * numTiles / 2.0F + 0.5F)), numTiles);
                    ...
                    int maxNumJobs = numThreads == 1 ? 1 : (int) ((float) numTiles
                            / (float) minTilesPerThread + 0.5F);
                    ...
                    while (numTilesLeft > 0) {
                        ...
                        jobs[numJobs] = new TileJob(this, isBlocking, owner, tileIndices, tiles,
    ...
    public Raster[] scheduleTiles(OpImage owner, Point tileIndices[]) {
        if (owner == null || tileIndices == null) {
            throw new IllegalArgumentException(JaiI18N.getString("JaiExtTileScheduler_0"));
        }
        return (Raster[]) scheduleJob(owner, tileIndices, true, false, null);
    }

    /**
     * Schedule a list of tiles for computation. The supplied listeners will be notified after each tile has been computed. This method ideally should
     * be non-blocking. If the <code>TileScheduler</code> implementation uses multithreading, it is at the discretion of the implementation which
     * thread invokes the <code>TileComputationListener</code> methods.
     */
    ...
    /**
     * Issues an advisory cancellation request to the <code>TileScheduler</code> stating that the indicated tiles of the specified image should not be
     ...
     * 
     * <p>
     * In this implementation of <code>TileScheduler</code> the second tile cancellation option is implemented, i.e., tiles are removed from the
    ...
    /**
     * Suggests to the scheduler the degree of parallelism to use in processing invocations of <code>scheduleTiles()</code>. For example, this might
     * set the number of threads to spawn. It is legal to implement this method as a no-op.
     * 
     * <p>
     * In this implementation of TileScheduler this method sets the number of worker threads actually used for tile computation. Ideally this number
     * should equal the number of processors actually available on the system. It is the responsibility of the application to set this value as the
     * number of processors is not available via the virtual machine. A parallelism value of zero indicates that all tile computation will be effected
     * in the primary thread. A parallelism value of <i>N</i> indicates that there will be <i>N</i> worker threads in addition to the primary
     * scheduler thread. In JAI the parallelism defaults to a value of 2 unless explicity set by the application.
     * 
    ...
    public void setParallelism(int parallelism) {
        if (parallelism < 0) {
            throw new IllegalArgumentException(JaiI18N.getString("JaiExtTileScheduler_2"));
        }
    ...
    /**
     * Suggests to the scheduler the priority to assign to processing effected by <code>scheduleTiles()</code>. For example, this might set thread
     ...
     * 
     * <p>
     * In this implementation of <code>TileScheduler</code>, this method sets the priority of any threads spawned to prefetch tiles. Its initial value
    ...
    /**
     * Recreate the <code>ThreadGroup</code>is and <code>WorkThread</code>s. This happens in the case of applet: the java plugin will exist after the
     * termination of the applet so that JAI and TileScheduler will also exist. However, the <code>ThreadGroup</code>s are destroyed. Thus, the old
     * workers should be terminated and new i <code>ThreadGroup</code> and workers should be created.
     */
    ...
    /**
     * Returns the effective number of threads of the specified type. This method also updates the number and priority of threads of the specified
     * type according to the global settings. This method may add <code>WorkerThread.TERMINATE</code>s to the appropriate queue if there are too many
     * effective threads.
     */
    private int getNumThreads(boolean isPrefetch) {
        ...
        if (nthr < prll) {
            ...
            while (nthr < prll) {
                Thread t = new WorkerThread(isPrefetch ? prefetchGroup : standardGroup, this,
                        isPrefetch);
