    ...
    protected void computeRect(Raster[] sources, WritableRaster dest, Rectangle destRect) {
        ...
        if (!hasROI || !roiDisjointTile) {
            ...
            if (colorModel instanceof IndexColorModel) {
                snbands = colorModel.getNumComponents();
            } else {
                snbands = sources[0].getNumBands();
            }
        ...
        if (caseA || (caseB && roiContainsTile)) {
            ...
            for (int h = 0; h < dstHeight; h++) {
                ...
                for (int w = 0; w < dstWidth; w++) {
                    ...
                    boolean valid = false;
                    for (int k = 0; k < dstBands; k++) {
                        short sample = ImageUtil.clampRoundUShort(srcData[k][srcPixelOffset
                                + srcBandOffsets[k]]);
                        if (!noData.contains(sample)) {
                            valid = true;
                        }
