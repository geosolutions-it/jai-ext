...
public class ColorIndexerTest extends TestBase {
	...
	public void test2BandsBug() {
		...
		RenderedOp img = BandMergeDescriptor
				.create(null, 0, null, image, image);
	...
	private RenderedImage quantize(RenderedImage image, ROI roi, Range nodata,
			int destNoData) {
		Quantizer q = new Quantizer(256);
		ColorIndexer indexer = q.buildColorIndexer(image);
		RenderedImage indexed = ColorIndexerDescriptor.create(image, indexer,
				roi, nodata, destNoData, null);
	...
	public void testGrayColorNoData() {
		...
		g.dispose();
		RenderedImage indexed = quantize(image, null,
				RangeFactory.create((byte) 255, (byte) 255), 1);
		assertTrue(indexed.getColorModel() instanceof IndexColorModel);
		IndexColorModel icm = (IndexColorModel) indexed.getColorModel();
	...
	public void testTranslatedImage() throws Exception {
		BufferedImage bi = new BufferedImage(256, 256,
				BufferedImage.TYPE_BYTE_GRAY);
		TiledImage image = new TiledImage(0, 0, 256, 256, 1, 1, bi
				.getSampleModel().createCompatibleSampleModel(256, 256),
				bi.getColorModel());
	...
	public void testFourColorROI() {
		...
		// create a palette out of it
		//
		RenderedImage indexed = quantize(image, new ROIShape(new Rectangle(10,
				0, 10, 10)), null, 10);
		assertTrue(indexed.getColorModel() instanceof IndexColorModel);
		IndexColorModel icm = (IndexColorModel) indexed.getColorModel();
	...
	public void testTranslatedImageTileGridROINoData() {
		BufferedImage image_ = new BufferedImage(256, 256,
				BufferedImage.TYPE_BYTE_GRAY);
		...
		TiledImage image = new TiledImage(0, 0, 256, 256, 128, 128, image_
				.getColorModel().createCompatibleSampleModel(256, 256),
		...
		component = BandCombineDescriptor.create(component, coeff, null, null,
				destinationNoData, null);

		StatsType[] stats = new StatsType[] { StatsType.EXTREMA };
		component = StatisticsDescriptor.create(component, 1, 1, null, null,
				false, new int[] { 0 }, stats, null);
		Statistics stat = ((Statistics[][]) component
				.getProperty(Statistics.STATS_PROPERTY))[0][0];
	...
	private RenderedImage forceComponentColorModel(RenderedImage image) {
		...
		final ComponentColorModel destinationColorModel = new ComponentColorModel(
				ColorSpace.getInstance(ColorSpace.CS_sRGB), bits, alpha, image
		...
		// Lookup Operations
		image = LookupDescriptor.create(image, new LookupTableWrapper(lut), 0,
				null, null, false, hints);

		return image;
	}

	private void checkNoDataROI(RenderedImage indexed, RenderedImage image,
		...
		RandomIter sourceIter = RandomIterFactory.create(image, null, true,
				true);
		RandomIter destIter = RandomIterFactory.create(indexed, null, true,
				true);
		...
		for (int y = minY; y < maxY; y++) {
			for (int x = minX; x < maxX; x++) {
				...
				// ROI Check
				if (roiExists
						&& !(roiBounds.contains(x, y) && roiIter.getSample(x,
								y, 0) > 0) && translated.contains(x, y) ) {
					valid = false;
				}
