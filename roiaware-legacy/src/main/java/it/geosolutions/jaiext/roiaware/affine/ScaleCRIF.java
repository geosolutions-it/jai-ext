...
public class ScaleCRIF extends CRIFImpl {
	...
	/**
	 * Creates a new instance of ScaleOpImage in the rendered layer. This method
	 * satisfies the implementation of RIF.
	 * 
	 * @param paramBlock
	 *            The source image, the X and Y scale factor, and the
	...
	public RenderedImage create(ParameterBlock paramBlock,
		...
		if (xScale == 1.0F && yScale == 1.0F && xTrans == 0.0F
				&& yTrans == 0.0F) {
		...
		if (xScale == 1.0F && yScale == 1.0F
			...
			return new TranslateIntOpImage(source, renderHints, (int) xTrans,
					(int) yTrans);
		}
		...
		try {
			...
			if (roi == null
					|| (ImageUtilities.isMediaLibAvailable() && (roi.getBounds().isEmpty() || roi.contains(sourceBounds)))) {
				RenderedImage accelerated = new MlibScaleRIF().create(
		...
		if (interp instanceof InterpolationNearest) {
			...
			if ((sm instanceof MultiPixelPackedSampleModel)
					...
					&& (sm.getDataType() == DataBuffer.TYPE_BYTE
							|| sm.getDataType() == DataBuffer.TYPE_USHORT || sm
							.getDataType() == DataBuffer.TYPE_INT)) {
				return new ScaleNearestBinaryOpImage(source, extender,
						renderHints, layout, xScale, yScale, xTrans, yTrans,
						interp);
			} else {
				return new ScaleNearestOpImage(source, extender, renderHints,
						layout, xScale, yScale, xTrans, yTrans, interp,
						useRoiAccessor);
			}
			...
			if ((sm instanceof MultiPixelPackedSampleModel)
					...
					&& (sm.getDataType() == DataBuffer.TYPE_BYTE
							|| sm.getDataType() == DataBuffer.TYPE_USHORT || sm
							.getDataType() == DataBuffer.TYPE_INT)) {
				return new ScaleBilinearBinaryOpImage(source, extender,
						renderHints, layout, xScale, yScale, xTrans, yTrans,
						interp, false);
			} else {
				return new ScaleBilinearOpImage(source, extender, renderHints,
						layout, xScale, yScale, xTrans, yTrans, interp,
						useRoiAccessor);
			}
		} else if ((interp instanceof InterpolationBicubic)
				|| (interp instanceof InterpolationBicubic2)) {
			return new ScaleBicubicOpImage(source, extender, renderHints,
					layout, xScale, yScale, xTrans, yTrans, interp);
		} else {
			return new ScaleGeneralOpImage(source, extender, renderHints,
					layout, xScale, yScale, xTrans, yTrans, interp);
		}
	}

	/**
	 * Creates a new instance of <code>AffineOpImage</code> in the renderable
	 * layer. This method satisfies the implementation of CRIF.
	 */
	 ...
	 * @param paramBlock
	 *            The ParameterBlock containing the sources and the translation
	 *            factors.
	 * @param image
	...
	/**
	 * Gets the bounding box for the output of <code>ScaleOpImage</code>. This
	 * method satisfies the implementation of CRIF.
	 */
