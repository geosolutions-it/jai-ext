...
/**
 * Creates a binary image based on tests of pixel inclusion in a polygonal
 * {@code Geometry}. See {@link VectorBinarizeDescriptor} for details.
 * 
...
public class VectorBinarizeOpImage extends SourcelessOpImage {
	 ...
	 * 
	 * @param sm
	 *            the {@code SampleModel} used to create tiles
	 * @param configuration
	 *            rendering hints
	 * @param minX
	 *            origin X ordinate
	 ...
	 *            image height
	 * @param geom
	 *            reference polygonal geometry
	 * @param antiAliasing
	 *            whether to use anti-aliasing when rendering the reference
	 *            geometry
	 */
	public VectorBinarizeOpImage(SampleModel sm, Map configuration, int minX,
			int minY, int width, int height, PreparedGeometry geom,
			boolean antiAliasing) {
		super(buildLayout(minX, minY, width, height, sm), configuration, sm,
	 ...
	 * {@code height} arguments are requested tile dimensions which will only be
	 ...
	 */
	static ImageLayout buildLayout(int minX, int minY, int width, int height,
			SampleModel sm) {
	...
	public Raster computeTile(int tileX, int tileY) {
		...
		Raster result = tile.createChild(0, 0, tileWidth, tileHeight, x, y,
				null);
		return result;
	}
	 ...
	 * constant {@code Raster} with 1 or 0 values is returned. Otherwise tile
	 * pixels are checked for inclusion and set individually.
	 * 
	 * @param minX
	 *            origin X ordinate
	 * @param minY
	...
	private Raster drawGeometry(final int minX, final int minY) {
		...
		SampleModel tileSampleModel = sampleModel.createCompatibleSampleModel(
				tileWidth, tileHeight);

		WritableRaster raster = RasterFactory.createWritableRaster(
				tileSampleModel, new java.awt.Point(0, 0));
		BufferedImage bi = new BufferedImage(colorModel, raster, false, null);
		Graphics2D graphics = null;
		try {
			graphics = bi.createGraphics();

			graphics.setClip(-offset, -offset, tileWidth + offset * 2,
					tileHeight + offset * 2);
	...
	/**
	 * Returns (creating and caching if the first call) a constant tile with 1
	 * values
	 * 
	...
	/**
	 * Returns (creating and caching if the first call) a constant tile with 0
	 * values
	 * 
	 * @return the constant tile
	 */
	...
	private Raster constantTile(int value) {
		...
		if (numBands != 1) {
			throw new IllegalArgumentException(
					"The code works only for single band rasters!");
		}
	...
	private Polygon getTestRect(int x, int y) {
		...
		ExtendedCoordinateSequence seq = new ExtendedCoordinateSequence(
				copyCoords);
