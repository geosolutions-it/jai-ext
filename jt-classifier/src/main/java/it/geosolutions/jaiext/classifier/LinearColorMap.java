...
/**
 * {@link ColorMapTransform} implementation defining a linear colormap
 * transformation
 * 
...
public final class LinearColorMap extends AbstractList<LinearColorMapElement>
	...
	public final static class LinearColorMapType {
		 ...
		 * @return <code>false</code> if we cannot valid the provided type,
	 ...
	 *            {@link LinearColorMap}.
	 */
	public LinearColorMap(final CharSequence name,
	...
	public LinearColorMap(final CharSequence name,
			final LinearColorMapElement[] standardElements, final Color defColor) {
	...
	public LinearColorMap(final CharSequence name,
		...
		if (preFilteringElements != null && preFilteringElements.length > 0) {
			this.preFilteringElements = (LinearColorMapElement[]) preFilteringElements
					.clone();
		...
		//
		// /////////////////////////////////////////////////////////////////////
		this.standardElements = (LinearColorMapElement[]) standardElements
				.clone();
	...
	private static void preliminarChecks(
		...
		final int num = domainElementsToPreserve != null ? domainElements.length
				+ domainElementsToPreserve.length
		...
		for (int i = 0; i < num; i++) {
			...
			for (int j = 0; j < num; j++) {
				...
				if (outRange1.intersects(outRange0)) {
					...
					if (minimum1 == maximum0
							&& colors0[colors0.length - 1].equals(colors1[0]))
						continue;

					if (minimum0 == maximum1
							&& colors1[colors1.length - 1].equals(colors0[0]))
						continue;
	...
	/**
	 * Returns a color model for this category list. This method builds up the
	 * color model from each category's colors (as returned by
	 * {@link #getColors} ).
	 * 
	 * @param visibleBand
	 *            The band to be made visible (usually 0). All other bands, if
	 *            any will be ignored.
	 * @param numBands
	 *            The number of bands for the color model (usually 1). The
	 *            returned color model will renderer only the
	 *            {@code        visibleBand} and ignore the others, but the existence
	 *            of all {@code        numBands} will be at least tolerated.
	 *            Supplemental bands, even invisible, are useful for processing
	 *            with Java Advanced Imaging.
	 * @return The requested color model, suitable for {@link RenderedImage}
	...
	private synchronized void initColorModel() {
		if (colorModel == null) {
			...
			int elementsCount = standardElements.length
					+ (preFilteringValuesPresent ? 1 : 0);
			int max = -1;
			...
			for (int i = 0; i < elementsCount; i++) {
				...
				ColorMapUtilities.expand(element.getColors(), ARGB, outMin,
						outMax + 1);
			}

			// create the prefiltering piecewise
			this.preFilteringPiecewise = preFilteringElements == null ? null
					: new DefaultPiecewiseTransform1D<LinearColorMapElement>(
							preFilteringElements);
			...
			if (lookForDefaultColor) {
				...
				for (int i = 0; i < ARGB.length; i++) {
					if (lookForDefaultColor && defaultColorIndex == -1
							&& bits.testBit(i)
			...
			colorModel = new IndexColorModel(
					ColorMapUtilities.getBitCount(max), max, ARGB, 0,
	...
	/**
	 * Creates a {@link SampleModel} compatible with the underlying
	 * {@link ColorModel} having the specified width and height.
	 * 
	 * @param width
	 *            represents the width for the final {@link SampleModel}
	 * @param height
	 *            represents the height for the final {@link SampleModel}
	 */
	public SampleModel getSampleModel(int width, int height) {
		...
		return new PixelInterleavedSampleModel(
				this.colorModel.getTransferType(), width, height, 1, width,
				new int[1]);
	}
	...
	public LinearColorMapElement get(int index) {
		...
		throw new ArrayIndexOutOfBoundsException(
				"Colormap Index is outside the colormap bounds");
	}

	public int size() {
		int size = this.preFilteringElements != null ? preFilteringElements.length
				: 0;
	...
	public boolean equals(Object o) {
		...
		if (preFilteringColor != that.preFilteringColor)
			return false;
		if (!PiecewiseUtilities.equals(preFilteringElements,
				that.preFilteringElements))
			return false;
