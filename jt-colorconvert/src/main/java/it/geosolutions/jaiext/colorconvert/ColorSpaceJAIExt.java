...
/**
 * Abstract class extending the {@link ColorSpaceJAI} class in order to add
 * support for external ROI or NoData.
 * 
...
public abstract class ColorSpaceJAIExt extends ColorSpaceJAI {
	...
	protected ColorSpaceJAIExt(int type, int numComponents,
	...
	public abstract WritableRaster fromCIEXYZ(Raster src,
			int[] srcComponentSize, WritableRaster dest,
			int[] dstComponentSize, ROI roi, Range nodata, float[] destNodata);

	public abstract WritableRaster fromRGB(Raster src, int[] srcComponentSize,
	...
	public abstract WritableRaster toRGB(Raster src, int[] srcComponentSize,
			WritableRaster dest, int[] dstComponentSize, ROI roi, Range nodata,
			float[] destNodata);

	@Override
	public WritableRaster fromCIEXYZ(Raster src, int[] srcComponentSize,
			WritableRaster dest, int[] dstComponentSize) {
		return fromCIEXYZ(src, srcComponentSize, dest, dstComponentSize, null,
				null, null);
	}
	...
	public WritableRaster fromRGB(Raster src, int[] srcComponentSize,
			WritableRaster dest, int[] dstComponentSize) {
		return fromRGB(src, srcComponentSize, dest, dstComponentSize, null,
				null, null);
	}
	...
	public WritableRaster toCIEXYZ(Raster src, int[] srcComponentSize,
			WritableRaster dest, int[] dstComponentSize) {
		return toCIEXYZ(src, srcComponentSize, dest, dstComponentSize, null,
				null, null);
	}
	...
	public WritableRaster toRGB(Raster src, int[] srcComponentSize,
			WritableRaster dest, int[] dstComponentSize) {
		return toRGB(src, srcComponentSize, dest, dstComponentSize, null, null,
				null);
	}
	...
	public static void RGB2XYZ(float[] RGB, float[] XYZ) {
		...
		XYZ[0] = 0.45593763F * RGB[0] + 0.39533819F * RGB[1] + 0.19954964F
				* RGB[2];
		XYZ[1] = 0.23157515F * RGB[0] + 0.77905262F * RGB[1] + 0.07864978F
				* RGB[2];
		XYZ[2] = 0.01593493F * RGB[0] + 0.09841772F * RGB[1] + 0.78488615F
				* RGB[2];
	}

	public static void XYZ2RGB(float[] XYZ, float[] RGB) {
		RGB[0] = 2.9311227F * XYZ[0] - 1.4111496F * XYZ[1] - 0.6038046F
				* XYZ[2];
		RGB[1] = -0.87637005F * XYZ[0] + 1.7219844F * XYZ[1] + 0.0502565F
				* XYZ[2];
	...
	public static WritableRaster RGBToCIEXYZ(Raster src,
		...
		case DataBuffer.TYPE_BYTE:
			RGBToCIEXYZByte(srcUid, srcComponentSize, dest, destComponentSize,
					roiContainsTile, roiDisjointTile, roiIter, bounds, nodata,
					destNodata);
			break;
		...
		case DataBuffer.TYPE_DOUBLE:
			RGBToCIEXYZDouble(srcUid, srcComponentSize, dest,
					destComponentSize, roiContainsTile, roiDisjointTile,
					roiIter, bounds, nodata, destNodata);
			break;
		}
	...
	private static void RGBToCIEXYZByte(UnpackedImageData src,
			int[] srcComponentSize, WritableRaster dest,
			int[] destComponentSize, boolean roiContainsTile,
			boolean roiDisjointTile, RandomIter roiIter, Rectangle roiBounds,
		...
		double[] destNoDataFinal = getConvertedNodataB(isInt, destNodata,
		...
		if (!hasNodata && (!hasROI || hasROI && roiContainsTile)) {
			for (int j = 0; j < height; j++, rStart += srcLineStride, gStart += srcLineStride, bStart += srcLineStride) {
				for (int i = 0, rIndex = rStart, gIndex = gStart, bIndex = bStart; i < width; i++, rIndex += srcPixelStride, gIndex += srcPixelStride, bIndex += srcPixelStride) {
					...
					if (isInt) {
						...
						dstPixels[dIndex++] = 0.45593763 * R + 0.39533819 * G
								+ 0.19954964 * B;
						dstPixels[dIndex++] = 0.23157515 * R + 0.77905262 * G
								+ 0.07864978 * B;
						dstPixels[dIndex++] = 0.01593493 * R + 0.09841772 * G
								+ 0.78488615 * B;
					}
				}
			}
			...
			for (int j = 0; j < height; j++, rStart += srcLineStride, gStart += srcLineStride, bStart += srcLineStride) {
				...
				for (int i = 0, rIndex = rStart, gIndex = gStart, bIndex = bStart; i < width; i++, rIndex += srcPixelStride, gIndex += srcPixelStride, bIndex += srcPixelStride) {
					...
					if (!(roiBounds.contains(x0, y0) && roiIter.getSample(x0,
							y0, 0) > 0)) {
			...
			for (int j = 0; j < height; j++, rStart += srcLineStride, gStart += srcLineStride, bStart += srcLineStride) {
				...
				for (int i = 0, rIndex = rStart, gIndex = gStart, bIndex = bStart; i < width; i++, rIndex += srcPixelStride, gIndex += srcPixelStride, bIndex += srcPixelStride) {
					...
					if (notValid
							|| !(roiBounds.contains(x0, y0) && roiIter
									.getSample(x0, y0, 0) > 0)) {
						dstPixels[dIndex++] = destNoDataFinal[0];
						dstPixels[dIndex++] = destNoDataFinal[1];
						dstPixels[dIndex++] = destNoDataFinal[2];
	...
	private static void RGBToCIEXYZShort(UnpackedImageData src,
		...
		double[] destNoDataFinal = getConvertedNodata(isInt, destNodata,
				DataBuffer.TYPE_SHORT, normr, normg, normb, normx, normy, normz);
	...
	private static void RGBToCIEXYZInt(UnpackedImageData src,
		...
		double[] destNoDataFinal = getConvertedNodata(isInt, destNodata,
				DataBuffer.TYPE_INT, normr, normg, normb, normx, normy, normz);
	...
	private static void RGBToCIEXYZFloat(UnpackedImageData src,
		...
		double[] destNoDataFinal = getConvertedNodata(isInt, destNodata,
				DataBuffer.TYPE_FLOAT, 0, 0, 0, normx, normy, normz);
		...
		if (!hasNodata && (!hasROI || hasROI && roiContainsTile)) {
			...
			for (int j = 0; j < height; j++, rStart += srcLineStride, gStart += srcLineStride, bStart += srcLineStride) {
				for (int i = 0, rIndex = rStart, gIndex = gStart, bIndex = bStart; i < width; i++, rIndex += srcPixelStride, gIndex += srcPixelStride, bIndex += srcPixelStride) {
					...
					boolean notValid = nodata.contains(RGB[0])
							|| nodata.contains(RGB[1])
	...
	private static void RGBToCIEXYZDouble(UnpackedImageData src,
		...
		double[] destNoDataFinal = getConvertedNodata(isInt, destNodata,
				DataBuffer.TYPE_DOUBLE, 0, 0, 0, normx, normy, normz);
	...
	public static WritableRaster CIEXYZToRGB(Raster src,
		...
		/*
		 * if the parameter srcComponentSize is null, use the sample size of the
		 * source raster.
		 */
		if (srcComponentSize == null)
			srcComponentSize = srcSampleModel.getSampleSize();
		...
		/*
		 * if the parameter dstComponentSize is null, use the sample size of the
		 * source raster.
		 */
		SampleModel dstSampleModel = dest.getSampleModel();
		if (destComponentSize == null)
		...
		case DataBuffer.TYPE_FLOAT:
			...
			break;
		case DataBuffer.TYPE_DOUBLE:
			CIEXYZToRGBDouble(srcUid, srcComponentSize, dest,
					destComponentSize, roiContainsTile, roiDisjointTile,
					roiIter, bounds, nodata, destNodata);
			break;
		}
	...
	private static void CIEXYZToRGBByte(UnpackedImageData src,
			int[] srcComponentSize, WritableRaster dest,
			int[] destComponentSize, boolean roiContainsTile,
			boolean roiDisjointTile, RandomIter roiIter, Rectangle roiBounds,
			Range nodata, float[] destNodata) {
		...
		double[] destNoDataFinal = getConvertedNodataCIEXYZ(destNodata,
				DataBuffer.TYPE_BYTE, upperr, upperg, upperb, normx, normy,
				normz);
	...
	private static void CIEXYZToRGBShort(UnpackedImageData src,
		...
		double[] destNoDataFinal = getConvertedNodataCIEXYZ(destNodata,
				DataBuffer.TYPE_SHORT, upperr, upperg, upperb, normx, normy,
				normz);
	...
	private static void CIEXYZToRGBFloat(UnpackedImageData src,
		...
		double[] destNoDataFinal = getConvertedNodataCIEXYZ(destNodata,
				DataBuffer.TYPE_FLOAT, upperr, upperg, upperb, 0, 0, 0);
		...
		if (!hasNodata && (!hasROI || hasROI && roiContainsTile)) {
			...
			for (int j = 0; j < height; j++, xStart += srcLineStride, yStart += srcLineStride, zStart += srcLineStride) {
				for (int i = 0, xIndex = xStart, yIndex = yStart, zIndex = zStart; i < width; i++, xIndex += srcPixelStride, yIndex += srcPixelStride, zIndex += srcPixelStride) {
					...
					boolean notValid = nodata.contains(XYZ[0])
	...
	private static void CIEXYZToRGBDouble(UnpackedImageData src,
		...
		double[] destNoDataFinal = getConvertedNodataCIEXYZ(destNodata,
				DataBuffer.TYPE_DOUBLE, upperr, upperg, upperb, 0, 0, 0);
	...
	private static double[] getConvertedNodataB(boolean isInt,
			float[] destNodata, int normr, int normg, int normb, double normx,
			double normy, double normz) {
	...
	private static double[] getConvertedNodata(boolean isInt,
			float[] destNodata, int dataType, double normr, double normg,
			double normb, double normx, double normy, double normz) {
	...
	private static double[] getConvertedNodataCIEXYZ(float[] destNodata,
			int dataType, double upperr, double upperg, double upperb,
