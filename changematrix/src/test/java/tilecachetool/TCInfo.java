...
public final class TCInfo extends JPanel implements ChangeListener
{
    ...
    public TCInfo()
    {
        ...
        memoryCapacitySlider = new JSlider(JSlider.VERTICAL, 0, 1024, 16);
        ...
        for (int j = 0; j <= 100; j += 10)
        {
            ...
            mt_labels.put(Integer.valueOf(j), label);
        }
    ...
    public void setTileCache(SunTileCache cache)
    {
        setTileCache(cache, false);
     ...
     * @since TCT 1.0
     */
    public void setTileCache(SunTileCache c, boolean maxValueAsMemoryCapacity)
    {
        if (c == null)
        {
        ...
        if (maxValueAsMemoryCapacity)
        {
            memoryCapacitySlider.setMaximum((int) (cache.getMemoryCapacity() / (1024L * 1024L)));
        }
