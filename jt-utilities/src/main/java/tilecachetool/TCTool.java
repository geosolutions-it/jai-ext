 ...
 * @version 1.01
 *
 * NOTE: The act of observing can change the observed behavior!!! This tool will
 * impact performance and will use memory from the JVM which interacts with the
 * garbage collector. An attempt was made to minimize these perturbations in
 * order to provide a useful method of monitoring and understanding the JAI tile
 * cache.
 */
...
public final class TCTool extends JFrame implements ActionListener, Observer
{
    ...
    private static final LineBorder LINE_BORDER = new LineBorder(
            Color.darkGray, 1);
    ...
    // about box message
    private static final String about_msg = "<html>" +
        ...
        "<center>All Rights Reserved</center>" + "<br></br>" + "</html>";
    ...
    /** Constructor with a non-default memory capacity */
    public TCTool(long memoryCapacity, SunTileCache sunTileCache)
    {
        create(memoryCapacity, sunTileCache);
    }
    ...
    /** Constructor with a custom tile cache */
    public TCTool(SunTileCache sunTileCache, final boolean maxValueAsMemoryCapacity)
    {
        create(-1, sunTileCache, maxValueAsMemoryCapacity);
    }
    ...
    {
        create(memoryCapacity, sunTileCache, false);
    }

    private void create(long memoryCapacity, SunTileCache sunTileCache, boolean maxValueAsMemoryCapacity)
    {
         ...
         * maintained "as is", so if the tile cache is currently enabled, then a
         * call is made to disable it, ops/tiles that were in the cache prior to
         * the call will still be cachable. New tile requests will not be
         * cached. The TCTool will still monitor items in the cache that were
         * there prior to the "disable" call. Likewise, it the cache is
         * currently disabled then enabled, tiles prior to the enable call will
         * not be cached or monitored. The tile cache is flushed when a call is
         * made to JAI.disableDefaultTileCache(), but any valid tiles that
         * existed will be recomputed and cached again.
         */
        ...
        // Tile Cache Options
        information.setTileCache(cache, maxValueAsMemoryCapacity);
        ...
        Object possibleCachedTile)
    {
        ...
        float memoryCapacity = (float) sun_tile_cache.getMemoryCapacity();
        float memoryUsage = (float) sun_tile_cache.getCacheMemoryUsed();
        ...
        eventData.addElement(String.valueOf(tileSize));
        eventData.addElement(String.valueOf(timeStamp));
        ...
        SwingUtilities.invokeLater(new Runnable()
            {
                public void run()
                {
                    ...
                    statistics.set(tileCount, cacheHits, cacheMisses, f_addTile,
                        f_removeTile, f_removeFlushed, f_removeMemoryControl,
                        f_updateAddTile, f_updateGetTile, percentTCM);
                }
            });
    }
}
