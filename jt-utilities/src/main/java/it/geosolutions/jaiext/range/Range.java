...
/**
 * Abstract class used for checking if a selected value is inside the selected
 * Range. All the methods throw an {@link UnsupportedOperationException} but for
 * every subclass one of these methods is overridden with the correct
 * functionality. These 6 methods are different only for the data type used. In
 * this way it is possible to reach a better performance by using primitive
 * variables than generic. All the subclasses can contain a Range composed by a
 * minimum and a maximum or a single-point Range. For Double and Float data type
 * the NaN data can be used only with a single-point Range.
 */
public abstract class Range {

	public enum DataType {
		BYTE(Byte.class, DataBuffer.TYPE_BYTE), USHORT(Short.class,
				DataBuffer.TYPE_USHORT), SHORT(Short.class,
				DataBuffer.TYPE_SHORT), INTEGER(Integer.class,
				DataBuffer.TYPE_INT), FLOAT(Float.class, DataBuffer.TYPE_FLOAT), DOUBLE(
				Double.class, DataBuffer.TYPE_DOUBLE),
