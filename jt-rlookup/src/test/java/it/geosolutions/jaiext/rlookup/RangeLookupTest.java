...
public class RangeLookupTest extends TestBase {
	...
	public static void initialSetup() {
		...
		IMAGE_FILLER = true;
		images[0] = createTestImage(DataBuffer.TYPE_BYTE, WIDTH, WIDTH,
				(byte) 0, false);
		images[1] = createTestImage(DataBuffer.TYPE_USHORT, WIDTH, WIDTH,
				(short) 0, false);
		images[2] = createTestImage(DataBuffer.TYPE_SHORT, WIDTH, WIDTH,
				(short) 0, false);
		images[3] = createTestImage(DataBuffer.TYPE_INT, WIDTH, WIDTH, 0, false);
		images[4] = createTestImage(DataBuffer.TYPE_FLOAT, WIDTH, WIDTH, 0f,
				false);
		images[5] = createTestImage(DataBuffer.TYPE_DOUBLE, WIDTH, WIDTH, 0d,
				false);
	...
	public void byteToByteROI() {
		...
		Byte[] values = { 0, 1, 2, 3, 4 };
		RenderedImage srcImg = images[0];
		assertLookup(breaks, values, srcImg, DataBuffer.TYPE_BYTE, roi,
				defaultV);
	}
	...
	public void shortToShortROI() {
		...
		Short[] values = { -50, -10, 0, 10, 50 };
		RenderedImage srcImg = images[2];
		assertLookup(breaks, values, srcImg, DataBuffer.TYPE_SHORT, roi,
				defaultV);
	}
	...
	public void floatToFloatROI() {
		...
		Float[] values = { -50f, -10f, 0f, 10f, 50f };
		RenderedImage srcImg = images[4];
		assertLookup(breaks, values, srcImg, DataBuffer.TYPE_FLOAT, roi,
				defaultV);
	}
	...
	public void doubleToDoubleROI() {
		...
		Double[] values = { -50d, -10d, 0d, 10d, 50d };
		RenderedImage srcImg = images[5];
		assertLookup(breaks, values, srcImg, DataBuffer.TYPE_DOUBLE, roi,
				defaultV);
	}
	...
	public void ushortToUShortROI() throws Exception {
		...
		Short[] values = { 0, 1, 2, 3, 4 };
		RenderedImage srcImg = images[1];
		assertLookup(breaks, values, srcImg, DataBuffer.TYPE_USHORT, roi,
				defaultV);
	}
	 ...
	 * 
	 * @param breaks
	 *            source image breakpoints
	 * @param values
	 *            lookup values
	 * @param srcImg
	 *            source image
	 * @param destType
	 *            expected destination image data type
	 */
	...
	public static <T extends Number & Comparable<? super T>, U extends Number & Comparable<? super U>> RangeLookupTable<T, U> createTable(
		...
		r = RangeFactory.create(Double.NEGATIVE_INFINITY, false,
				breaks[0].doubleValue(), false);
		builder.add(r, values[0]);

		for (int i = 1; i < N; i++) {
			r = RangeFactory.create(breaks[i - 1].doubleValue(), true,
					breaks[i].doubleValue(), false);
			builder.add(r, values[i]);
		}

		r = RangeFactory.create(breaks[N - 1].doubleValue(), true,
	...
	private RenderedOp doOp(RenderedImage srcImg, RangeLookupTable table) {
		return doOp(srcImg, table, null, null);
	}
	...
	/**
	 * Tests that a destination image contains expected values given the source
	 * image and lookup table.
	 * 
	 * @param srcImg
	 *            source image
	 * @param table
	 ...
	 *            destination image
	 */
	private void assertImageValues(RenderedImage srcImg,
			RangeLookupTable table, RenderedImage destImg, ROI roi,
			Double defaultValue) {
		...
		do {
			do {
				...
				case DataBuffer.TYPE_BYTE:
					assertEquals(expectedVal.byteValue(),
							(byte) destIter.getSample());
					break;
				case DataBuffer.TYPE_SHORT:
					assertEquals(expectedVal.shortValue(),
							(short) destIter.getSample());
					break;
				case DataBuffer.TYPE_INT:
					assertEquals(expectedVal.intValue(), destIter.getSample());
					break;
				case DataBuffer.TYPE_FLOAT:
					assertEquals(expectedVal.floatValue(),
							destIter.getSampleFloat(), TOLERANCE);
					break;
				case DataBuffer.TYPE_DOUBLE:
					assertEquals(expectedVal.doubleValue(),
							destIter.getSampleDouble(), TOLERANCE);
					break;
				}
