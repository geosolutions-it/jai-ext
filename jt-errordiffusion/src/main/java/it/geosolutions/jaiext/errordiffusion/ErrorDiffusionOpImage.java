 ...
 * 
 * <p>
 * This <code>OpImage</code> performs dithering of its source image into a
 * single band image using a specified color map and error filter. For each
 * pixel in the source image the nearest entry in the color map is found and the
 * index of this entry is assigned to the <code>OpImage</code> at that location.
 * The color quantization error is calculated by mapping the index back through
 * the color map. The error in each band is then "diffused" to other neighboring
 * pixels in the source image according to the specified error filter.
 * 
...
public class ErrorDiffusionOpImage extends UntiledOpImage {
	...
	/**
	 * Constant indicating that the inner random iterators must pre-calculate an
	 * array of the image positions
	 */
	public static final boolean ARRAY_CALC = true;

	/**
	 * Constant indicating that the inner random iterators must cache the
	 * current tile position
	 */
	public static final boolean TILE_CACHED = true;

	/**
	 * Smallest float value which when added to unity will yield something other
	 * than unity.
	 */
	private static final float FLOAT_EPSILON = 1.192092896E-07F;

	/**
	 * Variables used in the optimized case of 3-band byte to 1-band byte with a
	 * ColorCube color map and a Floyd-Steinberg kernel.
	 */
	...
	/**
	 * The color map which maps the <code>ErrorDiffusionOpImage</code> to its
	 * source.
	 */
	 ...
	 * 
	 * @param kernel
	 *            The <code>KernelJAI</code> to examine.
	 ...
	 * 
	 * @param colorCube
	 *            The color cube to be used in dithering.
		...
		for (int band = 0; band < NBANDS; band++) {
			...
			while (gray < 256) {
				...
				while ((float) gray < threshold) {
					ditherTable[pTab++] = ((gray - repValue) << ERR_SHIFT)
							+ tableBase;
					gray++;
				}
	...
	private static ImageLayout layoutHelper(ImageLayout layout,
			RenderedImage source, LookupTableJAI colorMap) {
		// Create or clone the layout.
		ImageLayout il = layout == null ? new ImageLayout()
		...
		if (colorMap.getNumBands() == 1 && colorMap.getNumEntries() == 2
			...
			sm = new MultiPixelPackedSampleModel(DataBuffer.TYPE_BYTE,
					il.getTileWidth(source), il.getTileHeight(source), 1);
		...
		if (sm.getNumBands() != 1) {
			sm = RasterFactory.createComponentSampleModel(sm,
					sm.getTransferType(), sm.getWidth(), sm.getHeight(), 1);
			il.setSampleModel(sm);
		...
		for (int i = 0; i < numColorMapBands; i++) {
			maxIndex = Math.max(
					colorMap.getOffset(i) + colorMap.getNumEntries() - 1,
					maxIndex);
		}
		...
		if ((maxIndex > 255 && sm.getDataType() == DataBuffer.TYPE_BYTE)
			...
			int dataType = maxIndex > 65535 ? DataBuffer.TYPE_INT
					: DataBuffer.TYPE_USHORT;
			sm = RasterFactory.createComponentSampleModel(sm, dataType,
					sm.getWidth(), sm.getHeight(), 1);
			il.setSampleModel(sm);
		...
		if ((layout == null || !il.isValid(ImageLayout.COLOR_MODEL_MASK))
				...
				&& (sm.getDataType() == DataBuffer.TYPE_BYTE || sm
	 ...
	 * 
	 * <p>
	 * The image dimensions are derived from the source image. The tile grid
	 * layout, SampleModel, and ColorModel may optionally be specified by an
	 * ImageLayout object. The calculation assumes that the entire color
	 * quantization error is distributed to the right and below the current
	 * pixel and the filter kernel values are handled appropriately.
	 * 
	 * @param source
	 *            A RenderedImage.
	 * @param layout
	 ...
	 * @param colorMap
	 *            The color map to use which must have a number of bands equal
	 *            to the number of bands in the source image. The offset of this
	 *            <code>LookupTableJAI</code> must be the same for all bands.
	 * @param errorKernel
	 *            The error filter kernel. This must have values between 0.0 and
	 *            1.0. Only the entries to the right of and on the same row as
	 *            the key entry, and those entries below of the row of the key
	 *            entry are used; all other values are ignored. The values used
	 *            must sum to 1.0. Note that if a 1-by-1 error filter kernel is
	 *            supplied, the value of the unique kernel element is irrelevant
	 *            and the output of the algorithm will simply be the index in
	 *            the supplied color map of the nearest matching color to the
	 *            source pixel at the same position.
	 */
	public ErrorDiffusionOpImage(RenderedImage source, Map config,
			ImageLayout layout, LookupTableJAI colorMap, KernelJAI errorKernel,
			ROI roi, Range nodata, int destNoData) {
		...
		isOptimizedCase = (sampleModel.getTransferType() == DataBuffer.TYPE_BYTE
				...
				&& numBandsSource == 3 && colorMap instanceof ColorCube && isFloydSteinbergKernel(errorKernel));
	 ...
	 * 
	 * @param sources
	 *            The source image Raster.
	 * @param dest
	 *            A WritableRaster tile containing the area to be computed.
	 * @param destRect
	...
	protected void computeImage(Raster[] sources, WritableRaster dest,
		...
		if (hasROI) {
			...
			srcRectExpanded.setRect(srcRectExpanded.getMinX() - 1,
		...
		if (roiDisjointTile) {
			ImageUtil.fillBackground(dest, destRect,
					new double[] { destNoData });
			return;
		}

		if (isOptimizedCase) {
			computeImageOptimized(source, dest, destRect, roiIter,
					roiContainsTile);
		} else {
			computeImageDefault(source, dest, destRect, roiIter,
					roiContainsTile);
		}
	}

	protected void computeImageDefault(Raster source, WritableRaster dest,
			Rectangle destRect, RandomIter roiIter, boolean roiContainsTile) {
		...
		int diffuseBelow = errorKernel.getHeight() - errorKernel.getYOrigin()
				- 1;
		int kernelOffsetRight = errorKernel.getYOrigin() * kernelWidth
		...
		if (caseA || (caseB && roiContainsTile)) {
				...
				for (int x = startX, z = 0; x <= endX; x++) {
					...
					for (int b = 0; b < numBandsSource; b++) {
						...
						if (currentPixel[b] < minPixelValue
							...
							currentPixel[b] = java.lang.Math.max(
									currentPixel[b], minPixelValue);
							currentPixel[b] = java.lang.Math.min(
									currentPixel[b], maxPixelValue);
						}
					}
					...
					if (isQuantizationError) {
						...
						for (int u = 1; u <= rightCount; u++) {
							for (int b = 0; b < numBandsSource; b++) {
								currentLine[sampleOffset++] += qError[b]
										* kernelData[kernelOffset];
							}
							kernelOffset++;
						}
						...
						for (int v = 1; v <= diffuseBelow; v++) {
							...
							sampleOffset = z - (offsetLeft + 1)
									* numBandsSource;
							for (int u = 1; u <= count; u++) {
								for (int b = 0; b < numBandsSource; b++) {
									line[sampleOffset++] += qError[b]
											* kernelData[kernelOffset];
								}
								kernelOffset++;
							}
			...
			for (int y = startY; y <= endY; y++) {
				...
				for (int x = startX, z = 0; x <= endX; x++) {
					...
					// Save the index in the output data buffer.
					dstData[dstOffset++] = inROI ? nearestIndex : destNoData;
				...
				for (int x = startX, z = 0; x <= endX; x++) {
					...
					for (int b = 0; b < numBandsSource; b++) {
						...
						if (currentPixelReal[b] < minPixelValue
							...
							currentPixelReal[b] = java.lang.Math.max(
									currentPixelReal[b], minPixelValue);
							currentPixelReal[b] = java.lang.Math.min(
									currentPixelReal[b], maxPixelValue);
						}
						// NoData Check
	...
	protected void computeImageOptimized(Raster source, WritableRaster dest,
			Rectangle destRect, RandomIter roiIter, boolean roiContainsTile) {
		// Set X-coordinate range.
		int startX = minX;
		int endX = startX + width - 1;
		...
		RasterAccessor srcAccessor = new RasterAccessor(source, new Rectangle(
				startX, startY, source.getWidth(), source.getHeight()),
