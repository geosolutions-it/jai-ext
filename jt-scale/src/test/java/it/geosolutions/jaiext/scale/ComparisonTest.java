...
public class ComparisonTest extends TestBase {
    ...
    public void testOperation(int dataType, TestRoiNoDataType testType) {
        ...
        for (int is = 0; is <= 3; is++) {
            ...
            if (OLD_DESCRIPTOR) {
                image = javax.media.jai.operator.ScaleDescriptor.create(testImage, scaleX, scaleY,
                        xTrans, yTrans, interpolation, hints);
            } else {
                image = ScaleDescriptor.create(testImage, scaleX, scaleY, xTrans, yTrans, interpolation,
