...
import it.geosolutions.jaiext.roiaware.iterators.RandomIterFactory;
...
public abstract class ScaleOpImage extends GeometricOpImage {
    ...
    final static BorderExtender roiExtender = BorderExtender.createInstance(BorderExtender.BORDER_ZERO);
    ...
    public ScaleOpImage(RenderedImage source,
        ...
        super(vectorize(source), // vectorize() checks for null source.
              ...
              cobbleSources, extender, interp, null);
		...
		while (srcXSplits.hasMoreElements()) {
			...
			if (newSrcRect.width <= interp.getWidth()) {
				...
				if (extender == null) {
				    ...
				    sources[0] =
					source0.getExtendedData(wSrcRect, extender);
				    if (hasROI && useRoiAccessor) 
						rois[0] = srcROIImage.getExtendedData(wSrcRect, roiExtender); 
				}
			...
			if (newSrcRect.height <= interp.getHeight()) {
                            ...
                            if ((hDestRect.width > 0) &&
                            	...
                            	if (extender == null) {
                				    ...
                				    sources[0] =
                					source0.getExtendedData(hSrcRect, extender);
                				    if (hasROI && useRoiAccessor) 
                						rois[0] = srcROIImage.getExtendedData(hSrcRect, roiExtender); 
                				}
			...
			if ((newDestRect.width > 0) &&
				...
				if (extender == null) {
				    ...
				    sources[0] =
					source0.getExtendedData(newSrcRect, extender);
				    if (hasROI && useRoiAccessor) 
						rois[0] = srcROIImage.getExtendedData(newSrcRect, roiExtender); 
				}
			    ...
			    // map one more time
                            ...
                            if (RTDestRect.width > 0 &&
                            	...
                            	if (extender == null) {
                				    ...
                				    sources[0] =
                					source0.getExtendedData(RTSrcRect, extender);
                				    if (hasROI && useRoiAccessor) 
                						rois[0] = srcROIImage.getExtendedData(RTSrcRect, roiExtender); 
                				}
			    ...
			    BTSrcRect = mapDestRect(BTDestRect, 0);
                            ...
                            if (BTDestRect.width > 0 &&
                            	...
                            	if (extender == null) {
                				    ...
                				    sources[0] =
                					source0.getExtendedData(BTSrcRect, extender);
                				    if (hasROI && useRoiAccessor) 
                						rois[0] = srcROIImage.getExtendedData(BTSrcRect, roiExtender); 
                				}
			    ...
			    // LRTSrcRect may still be out of bounds
                            ...
                            if (LRTDestRect.width > 0 &&
                            	...
                            	if (extender == null) {
                				    ...
                				    sources[0] =
                					source0.getExtendedData(LRTSrcRect, extender);
                				    if (hasROI && useRoiAccessor) 
                						rois[0] = srcROIImage.getExtendedData(LRTSrcRect, roiExtender); 
                				}
