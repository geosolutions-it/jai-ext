...
public interface ImageFunctionJAIEXT extends ImageFunction {
	 ...
	 * @param roi
	 *            Optional ROI used for reducing calculations to a defined
	 *            region
	 * @param nodata
	 *            Optional NoData range to use for masking particular values
	 * @param destNoData
	 *            Value to set for pixels which are not accepted or are outside
	 *            ROI
	 * 
	...
	void getElements(double startX, double startY, double deltaX,
			...
			double[] imag, Rectangle destRect, ROI roi, Range nodata,
			float destNoData);
